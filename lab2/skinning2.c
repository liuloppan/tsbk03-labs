...
void DeformCylinder()
{
  ...
  for (row = 0; row < kMaxRow; row++)
  {
    for (corner = 0; corner < kMaxCorners; corner++)
    {
      ...
      // g_vertsOrg
			// g_vertsRes
			mat4 totPrimMat = IdentityMatrix();
			mat4 totInvMat = IdentityMatrix(); //the total matrix multiplication and its inverse
			vec3 v = SetVector(0.0f, 0.0f, 0.0f);

			//loop through bones
			for( int boneNum = 0; boneNum < kMaxBones ; boneNum++)
			{
				mat4 transMat, M; // = transMat;  //translationsmatris i vila, R behövs ej i detta fall
				vec3 relPos; //relative bone position
				mat4 primMat, invMat; // M', M inverse
				
				float weight = g_boneWeights[row][corner][boneNum];

				//get relative position
				relPos = VectorSub(g_bones[boneNum].pos,g_bones[boneNum-1].pos);

				//create all needed matrices from relative position
				transMat = T(relPos.x, relPos.y, relPos.z); //create matrix from relative pos aka the translation, no rot in rest
				primMat = Mult(transMat,g_bonesRes[boneNum].rot); // transformMat mult animated rotation
				invMat = T(-relPos.x, -relPos.y, -relPos.z); //inverse translation

				totPrimMat = Mult(totPrimMat, primMat); 
				totInvMat = Mult(invMat,totInvMat);

				M = Mult(totPrimMat, totInvMat);
				
				v = VectorAdd(v, ScalarMult( MultVec3(M,g_vertsOrg[row][corner]), weight)); 
